import { zodResolver } from "@hookform/resolvers/zod";
import { Scale } from "lucide-react-native";
import { useEffect } from "react";
import { Controller, useForm } from "react-hook-form";
import { Button, InputBase, InputDate, StyledText } from "@/components/ui";
import { useToast } from "@/components/ui/Toast";
import { useHealthData, useMemberContext } from "@/hooks";
import { registerWeight } from "@/services/api/health";
import { getAuthToken } from "@/services/utils/getAuthToken";
import { type WeightFormData, weightSchema } from "@/validators";
import { ModalPageWrapper } from "../../Common/ModalPageWrapper";
import {
  ButtonsWrapper,
  FormContentWrapper,
  InputsWrapper,
} from "../../styles";
import type { HealthFormModalProps } from "./HealthFormModal.types";

export function RegisterWeightModal({
  isVisible,
  onClose,
}: HealthFormModalProps) {
  const { showToast } = useToast();
  const { memberId } = useMemberContext();
  const { reload: reloadHealthData } = useHealthData();

  const {
    control,
    handleSubmit,
    reset,
    formState: { errors, isSubmitting },
  } = useForm<WeightFormData>({
    resolver: zodResolver(weightSchema),
    defaultValues: {
      weightKg: 0,
      recordedAt: undefined,
      notes: null,
    },
    mode: "onChange",
  });

  useEffect(() => {
    if (!isVisible) {
      reset();
    }
  }, [isVisible, reset]);

  const handleCancel = () => {
    reset();
    onClose();
  };

  const onSubmit = handleSubmit(async (data) => {
    try {
      const token = await getAuthToken();
      if (!token) {
        showToast("Você precisa estar autenticado", "error");
        return;
      }

      const response = await registerWeight(
        {
          weightKg: data.weightKg,
          recordedAt: data.recordedAt.toISOString(),
          notes: data.notes?.trim() || null,
        },
        token,
        memberId || undefined
      );

      if (response.success) {
        showToast("Peso registrado com sucesso!", "success");
        reloadHealthData();
        reset();
        onClose();
      } else {
        showToast(response.message || "Erro ao registrar peso", "error");
      }
    } catch (error) {
      showToast(
        error instanceof Error ? error.message : "Erro ao registrar peso",
        "error"
      );
    }
  });

  return (
    <ModalPageWrapper
      header={{
        title: "Registrar peso",
        description: "Atualize o seu peso atual",
        icon: <Scale color="black" size={20} />,
      }}
      isVisible={isVisible}
      onClose={onClose}
    >
      <FormContentWrapper>
        <InputsWrapper>
          <Controller
            control={control}
            name="weightKg"
            render={({ field: { onChange, value } }) => (
              <>
                <InputBase
                  keyboardType="numeric"
                  onChangeText={(text) => {
                    const num = Number.parseFloat(text.replace(",", "."));
                    onChange(Number.isNaN(num) ? 0 : num);
                  }}
                  placeholder="Peso em kg"
                  value={value > 0 ? value.toString() : ""}
                />
                {errors.weightKg && (
                  <StyledText color="error" variant="mediumRegular">
                    {errors.weightKg.message}
                  </StyledText>
                )}
              </>
            )}
          />
          <Controller
            control={control}
            name="recordedAt"
            render={({ field: { onChange, value } }) => (
              <>
                <InputDate
                  mode="datetime"
                  onChange={onChange}
                  placeholder="Data e horário da medição"
                  value={value}
                />
                {errors.recordedAt && (
                  <StyledText color="error" variant="mediumRegular">
                    {errors.recordedAt.message}
                  </StyledText>
                )}
              </>
            )}
          />
          <Controller
            control={control}
            name="notes"
            render={({ field: { onChange, value } }) => (
              <InputBase
                onChangeText={(text) => onChange(text.trim() || null)}
                placeholder="Observações"
                value={value || ""}
              />
            )}
          />
        </InputsWrapper>
        <ButtonsWrapper addPadding>
          <Button
            isLoading={isSubmitting}
            label="Confirmar"
            onPress={onSubmit}
            variant="primary"
          />
          <Button label="Cancelar" onPress={handleCancel} variant="outline" />
        </ButtonsWrapper>
      </FormContentWrapper>
    </ModalPageWrapper>
  );
}
